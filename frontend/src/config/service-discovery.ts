...
class ServiceDiscovery {
  ...
  private isKubernetesEnvironment(): boolean {
    ...
    const inCluster =
      window.location.hostname.includes(".cluster.local") ||
  ...
  private getKubernetesNamespace(): string {
    return (
      import.meta.env.VITE_K8S_NAMESPACE ||
  ...
  private getKubernetesEndpoints(): ServiceEndpoint[] {
    ...
    const serviceName =
      import.meta.env.VITE_API_SERVICE_NAME || "zamaz-api-service";
  ...
  private getConsulEndpoints(): ServiceEndpoint[] {
    const consulHost =
      import.meta.env.VITE_CONSUL_HOST || "consul.service.consul";
  ...
  private getStaticEndpoints(): ServiceEndpoint[] {
    ...
    if (apiHost) {
      endpoints.push({
        ...
        protocol:
          (import.meta.env.VITE_API_PROTOCOL as "http" | "https") || "https",
        priority: 1,
      });
    }
  ...
  private logConfiguration(): void {
    ...
    console.log(
      ...
      this.config.healthCheckInterval || "Disabled",
    );
  ...
  private async checkEndpointHealth(endpoint: ServiceEndpoint): Promise<void> {
    ...
    try {
      ...
      console.log(
        `🏥 Health check for ${endpoint.name}: ${endpoint.healthy ? "✅" : "❌"} (${latency}ms)`,
      );
    } catch (error) {
  ...
  getNextEndpoint(options?: ServiceDiscoveryOptions): ServiceEndpoint | null {
    ...
    switch (strategy) {
      case "round-robin":
        const endpoint =
          healthyEndpoints[this.currentEndpointIndex % healthyEndpoints.length];
        this.currentEndpointIndex++;
        return endpoint;

      case "random":
        return healthyEndpoints[
          Math.floor(Math.random() * healthyEndpoints.length)
        ];
  ...
  buildApiUrl(options?: ServiceDiscoveryOptions): string {
    ...
    if (!endpoint) {
      console.error(
        "No endpoint available, falling back to default API config",
      );
  ...
  async refreshHealth(): Promise<void> {
    const promises = this.config.endpoints.map((ep) =>
      this.checkEndpointHealth(ep),
    );
  ...
  async fetchConsulEndpoints(serviceName: string): Promise<ServiceEndpoint[]> {
    const consulUrl =
      import.meta.env.VITE_CONSUL_API_URL || "http://localhost:8500";

    try {
      const response = await fetch(
        `${consulUrl}/v1/health/service/${serviceName}?passing=true`,
      );
  ...
  async fetchKubernetesEndpoints(
    namespace: string,
    serviceName: string,
