...
class ZeroTrustAPIError(Exception):
    ...
    def __init__(
        self, code: str, message: str, details: Optional[str] = None, trace_id: Optional[str] = None
    ):
        self.code = code
...
class AuthenticationRequest:
    ...
    def __init__(
        self, email: str, password: str, mfa: Optional[str] = None, remember: bool = False
    ):
        self.email = email
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        data = {"email": self.email, "password": self.password, "remember": self.remember}
        if self.mfa:
            data["mfa"] = self.mfa
        return data
        ...
        try:
            ...
            if not response.ok:
                try:
                    ...
                    raise ZeroTrustAPIError(
                        "HTTP_ERROR", f"HTTP {response.status_code}: {response.text}"
                    )
...
class ZeroTrustUtils:
    ...
    def is_token_expiring_soon(
        expires_at: Union[str, datetime, int], threshold_seconds: int
    ) -> bool:
        """
    ...
    def is_retryable_error(error: Exception) -> bool:
        ...
        if isinstance(error, ZeroTrustAPIError):
            return (
                error.code in ["RATE_LIMITED"]
                or "timeout" in error.code.lower()
                or "network" in error.code.lower()
            )
        return True
